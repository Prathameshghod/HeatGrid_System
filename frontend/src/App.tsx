...
export default function Dashboard() {
  ...
  const handleSearch = async (placeName: string, lat: number, lng: number) => {
    ...
    const stageMessages: Record<string, string> = {
      buildings_fetched: 'Loading building footprints...',
      roads_fetched: 'Loading road network...',
      amenities_fetched: 'Analyzing local amenities...',
    };
    ...
    try {
        ...
        while (true) {
          ...
          for (const part of parts) {
            ...
            try {
              ...
              } else if (stageMessages[chunk.status]) {
                setLoadingMessage(stageMessages[chunk.status]);
              }
      ...
      if (err instanceof Error && err.name === 'AbortError') {
        alert('Analysis timed out after 2 minutes. Try a smaller area or restart the backend.');
      } else {
        alert('Backend error. Ensure the analysis API is running on port 8000.');
      }
    } finally {
